<?php
class Lois_compatibilityModel extends CI_Model{
	...
	function selectAllTask(){
        $query = $this->db->query("select * from lois_compatibility where archive !='1'")->result_array();
		return $query;
	}
    ...
    function updatearchive($id,$arr){
        $this->db->where('id',$id);
        return $this->db->update('lois_compatibility',$arr);
    }
	//insert
	function inserttask($array,$user,$pending,$sta,$pr_id,$c1,$c2){
		
		$this->db->trans_start();
		$this->db->insert('lois_compatibility',$array);
	...
	function selectTaskById($id=1){
		$arr = $this->db->from('lois_compatibility')->where('id',$id)->get()->row_array();
		return $arr;
	}
	...
	function updatetask($id,$pr_id,$task,$user,$pending,$sta,$num,$c1,$c2,$str){
		...
		$this->db->trans_start ();
		$this->db->where ( 'id', $id );
		$this->db->update ( 'lois_compatibility', $task );
    ...
    function search_pn($pn){
        $arr = $this->db->query("select * from lois_compatibility where pn like '%$pn%'")->result_array();
        return $arr;
    }
}