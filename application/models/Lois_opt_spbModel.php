<?php
class Lois_opt_spbModel extends CI_Model{
	...
	function selectAllTask(){
        $query = $this->db->query("select * from lois_opt_spb where archive !='1'")->result_array();
		return $query;
	}
    ...
    function updatearchive($id,$arr){
        ...
        return $this->db->update('lois_opt_spb',$arr);
    }
	//insert
	function inserttask($array,$user,$pending,$sta,$pr_id){
	
		$this->db->trans_start();
		$this->db->insert('lois_opt_spb',$array);
	...
	function selectTaskById($id=1){
		$arr = $this->db->from('lois_opt_spb')->where('TID',$id)->get()->row_array();
		return $arr;
	}
