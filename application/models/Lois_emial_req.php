<?php
class Lois_email_reqModel extends CI_Model{
	...
	function selectAllTask(){
		$this->db->select('*');
		$arr = $this->db->get('lois_email_req');
		return $arr->result_array();
	}
	//insert
	function inserttask($array){
		
		$this->db->insert('lois_email_req',$array);
		return  $this->db->insert_id();
	}
	//select Task By Id
	function selectTaskById($id=1){
		$arr = $this->db->from('lois_email_req')->where('ID',$id)->get()->row_array();
		return $arr;
	}
	
	function updatetask($id,$task){
		  $this->db->where('id',$id);
	      return $this->db->update('lois_email_req',$task);
	}
	//common
