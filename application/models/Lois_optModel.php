<?php
class Lois_optModel extends CI_Model {
	...
	function selectAllTask() {
        $query = $this->db->query("select * from lois_opt where archive !='1'")->result_array();
        return $query;
    ...
    function updatearchive($id,$arr){
        $this->db->where('OPTID',$id);
        return $this->db->update('lois_opt',$arr);
    }
	// insert
	function inserttask($array,$user,$pending,$sta,$pr_id,$c1,$c2){
		
		$this->db->trans_start();
		$this->db->insert('lois_opt',$array);
	...
	function selectTaskById($id = 1) {
		$arr = $this->db->from ( 'lois_opt' )->where ( 'OPTID', $id )->get ()->row_array ();
		return $arr;
	}
    ...
    $arr = $this->db->query("select * from lois_opt where pn like '%$pn%'")->result_array();
    return $arr;
}
}